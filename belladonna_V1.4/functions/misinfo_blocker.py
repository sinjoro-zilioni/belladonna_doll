...
async def plus(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if ag_key_holder(update.message.from_user.id, update):
        ...
        text = "user: {} has added {} to the blocklist."
        block_list.append(context.args[0])
        persist_update()
        LOGGER.info("user: %s added %s to the blocklist.", user, (context.args[0]))
...
async def minus(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if ag_key_holder(update.message.from_user.id, update):
        user = update.effective_user.id
        text = "user: {} has removed {} from the blocklist."
        block_list.remove(context.args[0])
        persist_update()
        await update.message.reply_text('detractus {} ex criteria'.format(context.args[0]))
        LOGGER.info("user: %s removed %s from the blocklist.", user, (context.args[0]))
        await context.bot.send_message(chat_id=LOG_CHANNEL, text=(text.format(user, str(context.args[0]))), parse_mode=ParseMode.HTML)
    else:
        await update.message.reply_text(locked_out_reply)
...
async def scanner(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if any(regulator):
        ...
        try:
            ...
            await update.message.reply_text('👁‍🗨 Unable to eliminate this message! Has this automaton been granted admin privilege?')
    else:
        pass
