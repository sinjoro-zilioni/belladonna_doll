...
from keys import LOG_CHANNEL, au_key, locked_out_reply
...
async def bestow_key(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if au_key_holder(update.message.from_user.id, update) and len(context.args) == 1:
        try:
            ...
            await update.message.reply_text(f'@{admin}  could not bestow key upon user: {id}')
    else:
        await update.message.reply_text(
        f"You must bear the AU key to conduct this transaction!"
    )
...
async def confiscate_key(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if au_key_holder(update.message.from_user.id, update) and len(context.args) == 1:
        try:
            ...
            await update.message.reply_text(f'@{admin} could not confiscate key from user: {id}')
    else:
        await update.message.reply_text(
        f"You must bear the AU key to conduct this transaction!"
    )
...
async def key_list(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if au_key_holder(update.message.from_user.id, update):
        gear_train = []
        info = (
            f"Here is the list of Ag key bearers: {keys} ")

        for trigger in range(0, len(keys), 25):
            gear_train.append(keys[trigger:trigger + 25])

        for gear in gear_train:
            await update.message.reply_text(info)
    else:
        await update.message.reply_text(f"You must bear the AU key to conduct this transaction!") 
